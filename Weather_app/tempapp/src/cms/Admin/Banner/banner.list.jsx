...
const Bannerlist = () => {
  ...
  const loadData = async () => {
    try {
      let response = await BannerSvc.listAllBannerData(10, 1);
                ...
                loading ? <>
                  ...
                  bannerData && bannerData.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>{item.status}</td>
                      <td>
                        <NavLink to={'/addmin/banner/' + item._id} className="btn btn-sm btn-info">Edit</NavLink>
                        <NavLink onClick={(e) => { e.preventDefault(); handleDelete(item._id) }} to={'/addmin/banner/' + item._id} className="btn btn-sm btn-danger">delete</NavLink>
                      </td>
                    </tr>
                  ))
                )
